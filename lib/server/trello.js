...
async function addCard(msgId, body, logger = dummyLogger) {
  ...
  const { statusCode, body: result } = await request.post("https://api.trello.com/1/cards", {
    qs: {
  ...
  logger.error(`Unexpected response for create trello card ${statusCode}, ${JSON.stringify(body)}`);
  return;
}
...
async function cardById(id, logger = dummyLogger) {
  const { statusCode, body } = await request.get(`https://api.trello.com/1/cards/${id}`, {
    qs: {
      fields: "shortUrl",
      list: true,
      members: true,
  ...
  logger.error(`Unexpected response for get card by id ${statusCode}, ${JSON.stringify(body)}`);
  return;
}

async function searchCard(correlationId, logger = dummyLogger) {
  const { statusCode, body } = await request.get("https://api.trello.com/1/search", {
    qs: {
      idBoard: boardId,
      list: true,
      query: correlationId,
  ...
  if (statusCode === 200 && body.cards) {
    logger.info(`Response from trello search with query ${correlationId} ${JSON.stringify(body)}`);
    return body.cards[0];
  }
  logger.error(`Unexpected response for trello search ${statusCode}, ${JSON.stringify(body)}`);
  return;
}
